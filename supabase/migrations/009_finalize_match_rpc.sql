-- Atomic, idempotent finalize_match RPC.
-- Reads canonical scores/answers from DB and determines winner with tiebreakers:
--   1. Correct count (more correct wins)
--   2. Accuracy (higher correct/attempts ratio wins)
--   3. Fewer attempts (rewards efficiency)
--   4. Speed (lower total time on correct answers wins)
--   5. True draw (all tiebreakers equal)
-- Both players can safely call this; if already finished it returns existing result.
